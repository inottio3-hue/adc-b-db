    ...
    if not api_key:
        ...
        if data:
            ...
            if not records:
                ...
                # ========================================================
                # 🔍 フィルタリング（修正箇所：アカウント名に変更）
                # ========================================================
                st.markdown("### 🔍 フィルタリング")
                # アカウント名のリストを取得
                all_account_names = display_df['アカウント名'].unique()
                
                # アカウント名のマルチセレクトを作成
                selected_accounts = st.multiselect("アカウント名で絞り込み", options=all_account_names)
                
                # 選択されたアカウントがある場合、そのアカウントの行だけを残す
                if selected_accounts:
                    display_df = display_df[display_df['アカウント名'].isin(selected_accounts)]
                ...
                # キャンペーンリストに「全体合計」を追加
                # 注意: 上のフィルタでdisplay_dfが絞り込まれているため、
                # 自動的に「選択したアカウントのキャンペーン」だけがリストアップされます
